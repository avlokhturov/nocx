 ...
 *
 * Source: contracts/ports.sample.schema.json
...
/**
 * Result of the ports.sample JSON-RPC method — a manual retry that clears a terminal refusal and samples immediately: the discovery state for one authenticated target plus every forward the backend currently tracks. The single declaration of this shape: the renderer's TypeScript type is generated from it and the Go transport is validated against it.
 */
export interface PortsSampleResult {
  /**
