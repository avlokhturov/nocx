...
export async function mountTabManager(
  ...
  const pc = { listProfiles: vi.fn().mockResolvedValue([]), listGroups: vi.fn().mockResolvedValue([]) }
  const { TabManager } = await import('../tabs')
  const manager = new TabManager(bar, panes, c as unknown as import('../ipc').WSClient, cb, g, bn, pc as unknown as import('../profiles').ProfileClient)
  await vi.waitFor(() => {
