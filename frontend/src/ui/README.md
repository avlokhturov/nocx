...
| **CompletionDropdown** | `completion-dropdown.ts` (vanilla-emitted, lives beside the editor it floats over) | `ui-completion-dropdown` + `__list/__row/__source/__match/__footer`                                                                   | `data-open`; rows reuse kit `ui-collection-row` with the `data-selected` variance from collection-view.css; the source caption reuses `ui-badge` tones; `__match` highlights the matched prefix inside `displayText` — displayed, never inserted; `data-empty` on the one non-selectable row that explains zero candidates (no footer in that state); width follows the longest row (capped) and `left` is set inline from the caret anchor, so the panel never spans the pane |
