...
| **RecallPanel**        | `recall.ts` (vanilla-emitted, lives beside the editor it floats over)              | `ui-recall-panel` + `__header/__bar/__filter/__coverage/__list/__time/__detail/__detail-item/__detail-term/__detail-value/__hint/__footer` | `data-open`; rows reuse kit `ui-collection-row` with the `data-selected` variance defined in collection-view.css; empty state reuses `ui-empty-state`; rung/source captions reuse `ui-badge` tones; the filter bar is a display of keys the overlay captured — there is no input element                                                                                                                    |
