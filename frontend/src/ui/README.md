...
| **TreeRow**            | `tree-row.tsx`                                                                     | `ui-tree-row`, `ui-tree-row__disclosure`, `__name`, `__badge`, `__icon`                                                                                                              | the file tree's row: `data-depth` (indentation driven by the number, not nested DOM); `data-kind` regular \| dir \| symlink \| other \| unreadable and `data-link-kind` in the same vocabulary — these are the WIRE's words (design §5.1 `Kind`), so the panel passes an entry's kind through untranslated and there is no mapping layer for a defect to live in; `other` (FIFO, socket, device) lists as a leaf; `data-cyclic` (a cyclic symlink renders as a leaf, whatever toggle is supplied); `data-disclosure` expanded \| collapsed \| leaf — the disclosure is a native button part (`__disclosure`) with `aria-expanded`, Enter/Space operable, disabled while `data-busy`; `data-selected`, `data-focused`, `data-disabled` (permission denied renders dimmed, never a silent row); name ellipsises without clipping a glyph; optional trailing `badge` slot |
| **EditableRowList**    | `row-list.tsx`                                                                     | `ui-row-list` + `__row/__content/__remove/__empty/__add`                                                                                                                             | repeating editable row list — the kit's answer to hand-rolled row stacks; each row renders the caller's kit fields, a per-row remove, and a foot add; controlled (caller owns the data), `emptyLabel` above the add control when empty …                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| **ContextMenu**        | `context-menu.tsx`                                                                 | `ui-context-menu`, `ui-context-menu__item`                                                                                                                                           | the kit's menu primitive: a non-modal popover of actions anchored at a point — the one vocabulary for "right-click a thing, pick an action" (the Files panel's copy-path / Show-in-Finder menu is its first consumer). Owns the shell (fixed at x/y, clamped to the viewport), the `__item` native buttons, dismissal (outside pointerdown, Escape, picking an item) and the keyboard walk (arrows/Home/End, first item focused on open). Non-modal on purpose: clicking outside closes it, never traps focus. The caller supplies `{id, label, onSelect}` items and the anchor; the component never knows what the actions mean.                                                                                                                                                                                                                                      |
...
| Popover/Menu/Combobox | **ContextMenu**                | The menu half is built (`context-menu.tsx`) for the Files panel's row actions; popover and combobox still await a real consumer. |
