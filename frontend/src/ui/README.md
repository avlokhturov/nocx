...
| **TextField**        | `text-field.tsx`                                                      | `ui-text-field`, `ui-text-field__input`, `__unit`, `__caption`          | input types text \| number \| password; `data-multiline` → `<textarea>`; composes Field for label/desc/error; number fields: `unit` suffix inside the control, `caption` slot beneath (error replaces the caption in the same slot, so there is never a second line), `captionAlign` start \| end follows the column the field sits in |
...
| **RecallPanel**      | `recall.ts` (vanilla-emitted, lives beside the editor it floats over) | `ui-recall-panel` + `__header/__list/__time/__hint/__footer`            | `data-open`; rows reuse kit `ui-collection-row` with the `data-selected` variance defined in collection-view.css; empty state reuses `ui-empty-state`; rung/source captions reuse `ui-badge` tones                                                                                                                                     |
