...
| **RecallPanel**        | `recall.ts` (vanilla-emitted, lives beside the editor it floats over)              | `ui-recall-panel` + `__header/__search/__coverage/__list/__time/__detail/__detail-item/__detail-term/__detail-value/__match/__footer` | `data-open`; rows reuse kit `ui-collection-row` with the `data-selected` variance defined in collection-view.css; empty state reuses `ui-empty-state`; rung/source captions reuse `ui-badge` tones; the bottom-edge search row is `createSearchFieldDisplay` (kit `ui-search-field` identities) with the coverage line placed at its right end — the field carries the captured filter, it does not accept focus (the editor's arbiter owns every key)                                                                                                                                                                                                   |
| **CompletionDropdown** | `completion-dropdown.ts` (vanilla-emitted, lives beside the editor it floats over) | `ui-completion-dropdown` + `__list/__row/__source/__match/__group/__footer`                                                           | `data-open`; rows reuse kit `ui-collection-row` with the `data-selected` variance from collection-view.css; the source caption reuses `ui-badge` tones; `__match` highlights the matched prefix inside `displayText` — displayed, never inserted; `__group` is the history-section caption in a mixed list (a hairline + label, so a whole-line history row never reads as one more path row); `data-empty` on the one non-selectable row that explains zero candidates (no footer in that state); width follows the longest row (capped, measured once per open list) and `left` is set inline from the caret anchor, so the panel never spans the pane |
