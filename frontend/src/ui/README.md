...
| **BlockReceipt**       | `block-receipt.ts` (vanilla-emitted, attached to the frozen block)                 | `ui-block-receipt` + `__row/__kind/__value/__row-error/__actions/__primary/__drop/__expired`                                                                                         | the after-submit capture receipt: appears attached to the block of the command that carried a credential, never above the prompt; non-modal (focus stays in the editor; ⇧⌘S enters review, Escape returns); one row per capture — the kind badge, the masked value from the redaction's prefix/suffix, an editable name (`ui-text-field__input`) pre-filled with the backend's suggestion, and a per-row drop; one primary action naming its scope (`Save` / `Save N`) saving every row still in play; row hover is reported so the host can emphasise that row's chip in the block; the capture window (ttlMs from the wire) retires the receipt with an honest line and no actions that could fail. Controls are the kit's own identities — this component only places them. |
