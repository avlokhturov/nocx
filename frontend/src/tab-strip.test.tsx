...
describe('the sandbox launch action (nocx-ftap1)', () => {
  const mountStrip = (strip: HorizontalTabStrip | VerticalTabStrip) => {
    const container = document.createElement('div')
    document.body.appendChild(container)
    strip.mount(container)
    return container
  ...
  it('is absent by default and follows sandbox.enabled live', () => {
    const strip = new HorizontalTabStrip()
    const container = mountStrip(strip)

    expect(container.querySelector('[aria-label="New sandboxed tab"]')).toBeNull()

    strip.setSandboxEnabled(true)
    expect(container.querySelector('[aria-label="New sandboxed tab"]')).not.toBeNull()

    strip.setSandboxEnabled(false)
    expect(container.querySelector('[aria-label="New sandboxed tab"]')).toBeNull()
  })

  it('sits immediately beside + and reports the press in both orientations', () => {
    for (const strip of [new HorizontalTabStrip(), new VerticalTabStrip()]) {
      strip.setSandboxEnabled(true)
      let presses = 0
      strip.onNewSandboxedTab = () => (presses += 1)
      const container = mountStrip(strip)

      const ordinary = container.querySelector<HTMLButtonElement>('[aria-label="New tab"]')
      const sandboxed = container.querySelector<HTMLButtonElement>(
        '[aria-label="New sandboxed tab"]',
      )
      expect(sandboxed, `missing ${strip.orientation} sandbox action`).not.toBeNull()
      expect(ordinary?.nextElementSibling).toBe(sandboxed)
