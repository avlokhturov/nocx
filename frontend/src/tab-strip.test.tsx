...
describe('the sandbox launch action (nocx-ftap1)', () => {
  ...
  const sandboxRow = (container: HTMLElement): HTMLElement | undefined => {
    container.querySelector<HTMLButtonElement>('[aria-label="More"]')!.click()
    return [...document.querySelectorAll<HTMLElement>('.ui-context-menu__item')].find((row) =>
      row.textContent?.includes('New sandboxed tab'),
    )
  }

  it('is absent by default and follows sandbox.enabled live inside More', () => {
    const strip = new HorizontalTabStrip()
    const container = mountStrip(strip)

    expect(sandboxRow(container)).toBeUndefined()
    strip.setSandboxEnabled(true)
    expect(sandboxRow(container)).toBeDefined()
    strip.setSandboxEnabled(false)
    expect(sandboxRow(container)).toBeUndefined()
  })

  it('is a named More-menu row and reports the press in both orientations', () => {
    for (const strip of [new HorizontalTabStrip(), new VerticalTabStrip()]) {
      ...
      expect(row, `missing ${strip.orientation} sandbox action`).toBeDefined()
      row!.click()
      expect(presses).toBe(1)
