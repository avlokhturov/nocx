...
4. The backend reads both baselines from one atomic settings snapshot and requires the supplied revision to match. A missing, malformed, oversized, vanished, or conflicting persisted baseline fails closed as `-32007 setup-failed`; it is never silently narrowed.
...
7. Request/delta faults map to `-32602`. Persisted-baseline faults map to `-32007`. Error messages, stable reasons, and logs contain no user path.
