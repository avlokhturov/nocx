...
**AD-11 — Per-tab filesystem sandbox: renderer requests, backend authorizes (experimental, [ADR-0033](decisions/0033-native-per-tab-filesystem-sandbox.md), amended by [ADR-0034](decisions/0034-sandbox-writable-allowlist-and-launch-overrides.md), [ADR-0036](decisions/0036-sandbox-two-class-directory-grants.md), and [ADR-0037](decisions/0037-sandbox-launches-an-interactive-shell.md); ADR-0037 supersedes ADR-0035's fixed-OpenCode launch target).**

- Binds: `internal/sandbox` + local-PTY creation only; typed settings; the frontend Quick Connect and pre-launch permission surfaces; the `open`/`sandbox.status`/native-directory-dialog control plane.
