...
- Product invariant: the `sandbox.enabled`-gated shield in the active sidebar panel header converts the active local terminal tab in its verified current folder. Conversion creates a sandboxed local shell, waits for durable creation, moves its tab into the source tab's strip position, and only then closes the source. A pane carrying `sandbox_grants` is non-restorable by derivation: the backend startup sweep closes every granted open pane before the first `layout.read`, and the renderer independently refuses to adopt one. The command palette and tab-strip More menu contain no sandbox launch; `+`, `Cmd/Ctrl+T`, ordinary local panes, and SSH panes are unchanged.
