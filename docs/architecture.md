...
**AD-11 — Per-tab filesystem sandbox: renderer requests, backend authorizes (experimental, [ADR-0030](decisions/0030-native-per-tab-filesystem-sandbox.md), amended by [ADR-0031](decisions/0031-sandbox-writable-allowlist-and-launch-overrides.md)).**
...
- Prevents: renderer-authored policy; stale or unseen global grants; mutable grants on a running tab; and a second policy owner beside `internal/sandbox`.
- Rule: a present strict `open.sandbox` object requests a canonical workspace, one atomic settings revision, and bounded per-launch additions/removals. The backend reads the global `sandbox.allowedWritablePaths` baseline from that exact settings snapshot, validates every directory, and alone composes the immutable effective policy. The renderer never sends global/effective/Git/runtime/native-backend roots. Omitted `sandbox` means an ordinary launch; present `null`, malformed, duplicate, unknown, or stale input is rejected. Sandboxed sessions start in the canonical workspace.
- Product invariant: the `sandbox.enabled`-gated `Sandboxed shell…` action opens a **new** local tab after native workspace and permission confirmation. Ordinary local tabs, SSH tabs, the initial tab, `+`, and `Cmd/Ctrl+T` are untouched. Settings changes affect future launches only; running tabs and their copied metadata never mutate.
- Security invariant: opt-in sandbox launches **fail closed** — a request that cannot be validated or enforced is rejected (invalid request data uses `-32602`; availability/setup use reserved codes `-32010`/`-32011`/`-32012` with stable `data.reason`) and never degrades to an unsandboxed shell. A session is registered only after native enforcement is acknowledged: after Landlock restriction in the Linux helper, or from the nocx shim running inside the applied Seatbelt profile on macOS. User writable roots cannot equal or contain documented system read-only roots. The guarantee is filesystem-only; network, environment, credentials, IPC, devices, and processes stay outside the boundary.
