...
**AD-11 — Per-tab filesystem sandbox: renderer asks, backend owns (experimental, [ADR-0030](decisions/0030-native-per-tab-filesystem-sandbox.md)).**
...
- **Per-tab filesystem sandbox (experimental, [ADR-0030](decisions/0030-native-per-tab-filesystem-sandbox.md)).** Support envelope: Linux requires a kernel with **Landlock ABI >= 3** (fail closed below); macOS uses the **deprecated, runtime-probed `sandbox-exec`/Seatbelt**; **Windows is unsupported in V1**. Pull requests run real Landlock/Seatbelt enforcement smokes on their native runners. A release additionally invokes the built Linux binary and the executable inside the packaged macOS `.app`, proving the early helper/smoke dispatch exists in what ships. The full policy and lifecycle live in the [design spec](../.internal/specs/2026-08-02-native-filesystem-sandbox-design.md), not here.
