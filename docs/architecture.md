...
- Product invariant: the `sandbox.enabled`-gated shield sits in the activity bar's top zone beside the Files view icon and converts the active local terminal tab in its verified current folder. It is not an action in the Files panel header. The shield is disabled while a conversion is in flight. On a confirmed sandbox tab the shield stays visibly selected; pressing it again creates an ordinary local replacement in the verified current folder, waits for durable creation and transcript installation, preserves the strip position, and closes the sandbox pane. A failed replacement is closed without touching its source. Neither direction mutates a running process's authority. Before either replacement closes its source, the frontend transfers frozen command text, normal-buffer SGR scrollback, the unsent editor draft and selection into the replacement and inserts a visible new-shell boundary; PTY bytes never cross JSON and alternate-screen process state is not transferred. A sandboxed tab's title line starts with the shield marker before every other title or status marker. A pane carrying `sandbox_grants` is non-restorable by derivation: the backend startup sweep closes every granted open pane before the first `layout.read`, and the renderer independently refuses to adopt one. The command palette and tab-strip More menu contain no sandbox launch; `+`, `Cmd/Ctrl+T`, ordinary local panes, and SSH panes are unchanged.
- Security invariant: opt-in sandbox launches **fail closed as the complete product promise** — a request that cannot be validated, nocxified, granted, or enforced is rejected (`-32602` for invalid request data, `-32005` `feature-disabled`, `-32006` for backend/status unavailability, `-32007` `setup-failed`) and never degrades to an unsandboxed or conventional shell. The request must name an open layout pane without an existing sandbox grant; the backend records a provisional pane grant after policy realization and before helper start. A process-start or readiness failure removes that provisional grant through the serialized repository writer; native readiness makes it immutable. `workspaceId` remains provenance and never grants authority. A session is registered only after native enforcement is acknowledged: after Landlock restriction in the Linux helper, or from the nocx shim running inside the applied Seatbelt profile on macOS.
