...
**AD-11 — Per-tab filesystem sandbox: renderer requests, backend authorizes (experimental, [ADR-0033](decisions/0033-native-per-tab-filesystem-sandbox.md), amended by [ADR-0034](decisions/0034-sandbox-writable-allowlist-and-launch-overrides.md), [ADR-0036](decisions/0036-sandbox-two-class-directory-grants.md), [ADR-0037](decisions/0037-sandbox-launches-an-interactive-shell.md), [ADR-0038](decisions/0038-sandbox-denied-access-inbox.md), and [ADR-0039](decisions/0039-explicit-home-grants-project-into-isolated-home.md); ADR-0037 supersedes ADR-0035's fixed-OpenCode launch target).**

- Binds: `internal/sandbox` + local-PTY creation only; typed settings; the frontend Quick Connect, pre-launch permission, and Settings → Developer → Sandbox access surfaces; the `open`/`sandbox.status`/`sandbox.access.*`/native-directory-dialog control plane.
- Prevents: renderer-authored policy or launch command; stale or unseen global grants; mutable grants on a running tab; and a second policy owner beside `internal/sandbox`.
- Rule: a present strict `open.sandbox` object requests a canonical workspace, one atomic settings revision, and bounded class-scoped additions/removals. The backend reads both global `sandbox.allowedWritablePaths` and `sandbox.allowedReadOnlyPaths` baselines from that exact settings snapshot, validates every directory, and alone composes the immutable effective two-class policy. The renderer never sends a command, executable, global/effective/Git/runtime/native-backend root, or trusted executable. Omitted `sandbox` means an ordinary launch; present `null`, malformed, duplicate, unknown, or stale input is rejected. Sandboxed sessions start in the canonical workspace.
- Product invariant: the `sandbox.enabled`-gated `Sandboxed shell…` action opens a **new** interactive local login shell after native workspace and permission confirmation. The backend establishes the authenticated local nocxify channel from a private bootstrap artifact under the per-session runtime tree; the shell remains the foreground process. nocx does not choose or start an agent. Ordinary local tabs, SSH tabs, the initial tab, `+`, and `Cmd/Ctrl+T` are untouched. Settings changes affect future launches only; running tabs and their copied metadata never mutate.
- Security invariant: opt-in sandbox launches **fail closed as the complete product promise** — a request that cannot be validated, nocxified, or enforced is rejected (invalid request data uses `-32602`; availability/setup use reserved codes `-32010`/`-32011`/`-32012` with stable `data.reason`) and never degrades to an unsandboxed or conventional shell. A session is registered only after native enforcement is acknowledged: after Landlock restriction in the Linux helper, or from the nocx shim running inside the applied Seatbelt profile on macOS. User writable roots cannot equal or contain documented system read-only roots; an effective read-only root cannot equal or descend from an effective writable root; a writable child below a read-only ancestor is allowed as an explicit exception. Backend enforcement executables cannot live under a writable root. The guarantee is filesystem-only; network, environment, credentials, IPC, devices, and processes stay outside the boundary.
- Diagnostic invariant: denied-access observation is separate from authorization. Linux uses a descendant-inherited seccomp user-notification listener only to observe `openat`/`openat2`, always replies `CONTINUE`, and leaves Landlock authoritative; macOS tags the Seatbelt deny-default profile with a backend nonce and consumes matching unified-log reports. Observer failure never widens policy or falls back unsandboxed. A Linux failure after USER_NOTIF interposition exists rejects startup or terminates only that affected sandbox fail-closed so no syscall is left blocked; other unavailable/failure paths change monitor status only ([ADR-0038](decisions/0038-sandbox-denied-access-inbox.md)). Reports are accepted only through a per-incarnation recorder bound to backend-minted `{sessionId, instanceId, sessionEpoch}`; provisional reports become visible only after native readiness, and late reports after close are dropped.
- Privacy and promotion invariant: the denied-access inbox is bounded, coalesced, memory-only `PrivateMetadata`; it never contains PTY bytes, argv, environment, command text, or secret material. `sandbox.access.changed` is a path-free invalidation. The renderer resolves a backend-minted event ID with one closed decision; the backend revalidates the displayed existing directory and atomically appends it to the latest RO or RW global baseline. No current tab is mutated or retried.
- HOME projection invariant: the backend resolves and canonicalizes host HOME from the pre-sandbox command environment, then considers only the canonical workspace and surviving explicit global/per-tab read-only/read-write grants. A candidate projects only when it is a strict canonical descendant of host HOME and does not intersect the per-session runtime tree. `/`, host HOME, its ancestors, outside paths, and derived Git/system/PATH/runtime/shell/device roots remain absolute-only; a broad ancestor never implies a scan or child mapping.
- Projection authority invariant: a projection is a disposable discoverability alias, never an authorization rule. Native canonical target rules remain authoritative, including the writable-child-under-read-only-ancestor exception; projection metadata carries no access class. No host-HOME scan, content copy, credential inspection, bind mount, renderer-authored mapping, or application-name special case is allowed ([ADR-0039](decisions/0039-explicit-home-grants-project-into-isolated-home.md)).
...
- **Per-tab filesystem sandbox (experimental, [ADR-0033](decisions/0033-native-per-tab-filesystem-sandbox.md), [ADR-0034](decisions/0034-sandbox-writable-allowlist-and-launch-overrides.md), [ADR-0036](decisions/0036-sandbox-two-class-directory-grants.md), [ADR-0037](decisions/0037-sandbox-launches-an-interactive-shell.md)).** Support envelope: Linux requires a kernel with **Landlock ABI >= 3** (fail closed below); macOS uses the **deprecated, runtime-probed `sandbox-exec`/Seatbelt** plus an in-profile readiness shim; **Windows is unsupported in V1**. Pull requests run real Landlock/Seatbelt enforcement smokes on their native runners. A release additionally invokes the built Linux binary and the executable inside the packaged macOS `.app`, proving the early helper/smoke dispatch exists in what ships. Sandboxed Quick Connect launches additionally require a bash/zsh local integration tier; the private nocxify artifact lives under the per-session runtime tree, and the authenticated interactive shell remains foreground after hello/accept. The full base policy and lifecycle live in the [V1 design](../.internal/specs/2026-08-02-native-filesystem-sandbox-design.md); the typed two-class baseline, strict launch DTO, revision gate, protected-root rule, and macOS readiness amendment live in the [permissions design](superpowers/specs/2026-08-16-sandbox-per-tab-permissions-design.md); the restored shell launch contract lives in the [shell restoration design](superpowers/specs/2026-08-18-sandbox-shell-contract-restoration-design.md).
