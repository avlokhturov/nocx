...
- Rule: a present strict `open.sandbox` object requests a canonical workspace, one atomic settings revision, and bounded class-scoped additions/removals. The backend reads both global `sandbox.allowedWritablePaths` and `sandbox.allowedReadOnlyPaths` baselines from that exact settings snapshot, validates every directory, and alone composes the immutable effective two-class policy and fixed `opencode` launch intent. The renderer never sends a command, executable, global/effective/Git/runtime/native-backend root, or trusted executable. Omitted `sandbox` means an ordinary launch; present `null`, malformed, duplicate, unknown, or stale input is rejected. Sandboxed sessions start in the canonical workspace.
- Product invariant: the `sandbox.enabled`-gated `Sandboxed opencode…` action opens a **new** local tab after native workspace and permission confirmation. The backend establishes the authenticated local nocxify channel before the bootstrap shell replaces itself with fixed backend-resolved `opencode`; the lifecycle descriptor remains inherited until agent exit. Ordinary local tabs, SSH tabs, the initial tab, `+`, and `Cmd/Ctrl+T` are untouched. Settings changes affect future launches only; running tabs and their copied metadata never mutate.
- Security invariant: opt-in sandbox launches **fail closed as the complete product promise** — a request that cannot be validated, nocxified, resolved to the fixed agent, or enforced is rejected (invalid request data uses `-32602`; availability/setup use reserved codes `-32010`/`-32011`/`-32012` with stable `data.reason`) and never degrades to an unsandboxed or conventional shell. A session is registered only after native enforcement is acknowledged: after Landlock restriction in the Linux helper, or from the nocx shim running inside the applied Seatbelt profile on macOS. User writable roots cannot equal or contain documented system read-only roots; an effective read-only root cannot equal or descend from an effective writable root; a writable child below a read-only ancestor is allowed as an explicit exception. Trusted executables cannot live under a writable root. The guarantee is filesystem-only; network, environment, credentials, IPC, devices, and processes stay outside the boundary.
...
- **Per-tab filesystem sandbox (experimental, [ADR-0033](decisions/0033-native-per-tab-filesystem-sandbox.md), [ADR-0034](decisions/0034-sandbox-writable-allowlist-and-launch-overrides.md), [ADR-0035](decisions/0035-sandboxed-opencode-is-a-fixed-authenticated-launch.md)).** Support envelope: Linux requires a kernel with **Landlock ABI >= 3** (fail closed below); macOS uses the **deprecated, runtime-probed `sandbox-exec`/Seatbelt** plus an in-profile readiness shim; **Windows is unsupported in V1**. Pull requests run real Landlock/Seatbelt enforcement smokes on their native runners. A release additionally invokes the built Linux binary and the executable inside the packaged macOS `.app`, proving the early helper/smoke dispatch exists in what ships. Sandboxed Quick Connect launches additionally require a bash/zsh local integration tier and a backend-resolved `opencode`; the private nocxify artifact lives under the per-session runtime tree, and the agent starts only after the authenticated hello/accept. The full base policy and lifecycle live in the [V1 design](../.internal/specs/2026-08-02-native-filesystem-sandbox-design.md); the typed writable baseline, strict launch DTO, revision gate, protected-root rule, and macOS readiness amendment live in the [permissions design](superpowers/specs/2026-08-16-sandbox-per-tab-permissions-design.md); the fixed launch flow lives in the [opencode launch design](superpowers/specs/2026-08-16-sandboxed-opencode-launch-design.md).
