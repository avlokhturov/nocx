...
**AD-11 — Per-tab filesystem sandbox: renderer requests, backend authorizes (experimental, [ADR-0033](decisions/0033-native-per-tab-filesystem-sandbox.md), amended by [ADR-0034](decisions/0034-sandbox-writable-allowlist-and-launch-overrides.md) and [ADR-0035](decisions/0035-sandboxed-opencode-is-a-fixed-authenticated-launch.md)).**
...
- Rule: a present strict `open.sandbox` object requests a canonical workspace, one atomic settings revision, and bounded per-launch additions/removals. The backend reads the global `sandbox.allowedWritablePaths` baseline from that exact settings snapshot, validates every directory, and alone composes the immutable effective policy and fixed `opencode` launch intent. The renderer never sends a command, executable, global/effective/Git/runtime/native-backend root, or trusted executable. Omitted `sandbox` means an ordinary launch; present `null`, malformed, duplicate, unknown, or stale input is rejected. Sandboxed sessions start in the canonical workspace.
