...
| -------- | --------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `open`   | **OpenOperation** (config, session)                 | Phase 0 validates the strict request and resolves `paneId → workspaceId`; sandbox opt-in additionally reads one atomic settings snapshot and requires the named open pane to be `ephemeral`, mapping `-32602/-32005/-32006/-32007` before spawn. `Prepare` resolves profile/vault state under `[config, session]`; `Dial` opens the PTY/SSH channel on the execution lane after those gates are released.                                     |
