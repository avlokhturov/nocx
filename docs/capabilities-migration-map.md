...
| -------- | --------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `open`   | **OpenOperation** (config, session)                 | Phase 0 validates the strict request and resolves `paneId → workspaceId`; sandbox opt-in additionally reads one atomic settings snapshot, resolves the effective standard/workspace profile, checks both displayed revisions, and requires the named open pane to have no existing sandbox grant, mapping `-32602/-32005/-32006/-32007` before spawn. After policy realization and before helper start it records the provisional pane grant with backend-owned profile provenance; native readiness makes it immutable. `Prepare` resolves profile/vault state under `[config, session]`; `Dial` opens the PTY/SSH channel on the execution lane after those gates are released. |
...
3. **`open` is two-phase** — `Prepare` holds `[config, session]` only while
   ...
   runs before both: one settings snapshot plus read-only pane/workspace and
   effective-profile resolution, never the gated layout writer. This keeps a
   slow handshake or native readiness wait from refusing unrelated
   config/session work.
