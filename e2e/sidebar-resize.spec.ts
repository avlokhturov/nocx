 ...
 * seam that persists it is the `sidebar.width` setting: drag → settings.set
 * → settings.json → restart → getSnapshot → applied to #sidebar. The
 * assertion after the restart is the panel's computed width, not a variable
 * read back.
 *
...
/** The persisted width in the backend's settings.json — the durable seam.
 *  The backend's HOME is the isolated home under the disposable root
 *  (root/home, per home-isolation.ts), so the doc lives under THAT home,
 *  never under the root itself. */
function persistedWidth(backend: VaultBackend): unknown {
  try {
    const doc = JSON.parse(
      readFileSync(join(documentDir(backend.isolatedHome), 'settings.json'), 'utf8'),
    ) as {
      values?: Record<string, unknown>
    }
    return doc.values?.['sidebar.width']
  } catch {
    return undefined
  }
}
