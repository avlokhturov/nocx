...
test.describe('agent ask about a frozen block (nocx-x8s2.2)', () => {
  ...
  test("point at a finished block, ask, and the answer streams in naming the block's output", async ({
    ...
    // agent.status now reads endpoint configured + credential resolvable.
    // The exact success sentence is deliberately not pinned: the form's
    // silent model-discovery probe records a lastProbe on the backend, and
    // whether it landed before this read is timing (AGENTS.md: no test may
    // depend on timing). Both success sentences — "Ready" and "Last test
    // ok (...)" — prove the credential resolved; the unavailable sentence
    // is the one this bead is about, and it must not appear.
    await expect(page.locator(STATUS_ROW)).toContainText(/Ready|Last test ok/, {
      timeout: 10_000,
    })
  ...
  test('the Test button on a saved endpoint probes with the STORED credential, and a typed key wins (nocx-reu5)', async ({
    ...
    await openApp(page)
    await openAIEndpoints(page)
    // Success sentence, not the exact one: an earlier test's silent
    // discovery probe may or may not have landed by now (AGENTS.md: no test
    // may depend on timing). "Ready" and "Last test ok" both prove the
    // credential resolves; the unavailable sentence is the failure this
    // section must not be in.
    await expect(page.locator(STATUS_ROW)).toContainText(/Ready|Last test ok/, {
      timeout: 10_000,
    })
