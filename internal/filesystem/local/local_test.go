...
func TestListPermissionDenied(t *testing.T) {
	...
	_, err := New().List(context.Background(), locked, filesystem.Page{Offset: 0, Limit: 10})
...
func TestListSymlinkToDirResolvesCanonicalAndListsThat(t *testing.T) {
	...
	l, err := New().List(context.Background(), link, filesystem.Page{Offset: 0, Limit: 10})
	if err != nil {
		t.Fatal(err)
	}
	if l.Canonical != real {
...
func TestListTimedOut(t *testing.T) {
	...
	p := New(WithListTimeout(time.Nanosecond))
	_, err := p.List(context.Background(), dir, filesystem.Page{Offset: 0, Limit: 10})
	...
	if _, err := New().List(context.Background(), dir, filesystem.Page{Offset: 0, Limit: 10}); err != nil {
...
func TestListInvalidPath(t *testing.T) {
	...
	_, err = New().List(context.Background(), dir+"/./", filesystem.Page{Offset: 0, Limit: 10})
	if !errors.As(err, &ixp) {
...
func TestRootDefaultIsInferredHome(t *testing.T) {
	...
	root, err := New().Root(context.Background())
	if err != nil {
		t.Fatal(err)
	}
	if root.Path != home {
		t.Errorf("Path = %q, want home %q", root.Path, home)
	}
...
func TestRootWithOverride(t *testing.T) {
	...
	if r.Path != dir || r.Inferred || r.InferredReason != "" {
		t.Errorf("verified root = %+v, want %q not inferred", r, dir)
	}
...
func TestReadRespectsMaxBytes(t *testing.T) {
	...
	c, err = New().Read(context.Background(), small, 100)
	if err != nil {
		t.Fatal(err)
	}
...
func TestReadDefaultLimitIs2MiB(t *testing.T) {
	...
	for _, maxBytes := range []int64{0, -1, 1 << 30} { // default, negative, huge → all 2 MiB
		c, err := New().Read(context.Background(), f, maxBytes)
		if err != nil {
			t.Fatal(err)
		}
...
func TestReadPermissionDenied(t *testing.T) {
	...
	_, err := New().Read(context.Background(), f, 0)
	var pe *filesystem.ErrPermission
	if !errors.As(err, &pe) {
...
func TestReadFIFORefused(t *testing.T) {
	...
	_, err := New().Read(context.Background(), fifo, 0)
	var nr *filesystem.ErrNotRegular
	if !errors.As(err, &nr) {
		t.Fatalf("err = %v, want ErrNotRegular", err)
	}
...
func TestReadSymlinkResolves(t *testing.T) {
	...
	c, err := New().Read(context.Background(), link, 0)
	if err != nil {
		t.Fatal(err)
	}
...
func TestCanonicalMethod(t *testing.T) {
	...
	link := filepath.Join(dir, "link")
	mustSymlink(t, real, link)
	got, err := New().Canonical(context.Background(), link)
	if err != nil {
		t.Fatal(err)
	}
	if got != real {
	...
	if _, err := New().Canonical(context.Background(), "rel"); !errors.As(err, &ixp2) {
...
// TestListKeepsCoherenceWhenParentRetargetedMidOperation: metadata is read
...
func TestListResponseSizeCeiling(t *testing.T) {
	...
	if _, err := New().List(context.Background(), long, filesystem.Page{Offset: 0, Limit: 10}); err != nil {
		t.Fatalf("default-cap listing failed: %v", err)
	}
	if _, err := New(WithSizeCap(0)).List(context.Background(), long, filesystem.Page{Offset: 0, Limit: 10}); err != nil {
...
// TestReadSwappedToFIFOBetweenCheckAndOpen: the old code statted the path
// and then opened it in two calls, leaving a window in which a regular file
// could become a FIFO — and os.Open on a FIFO blocks forever. The check and
...
func TestReadSwappedToFIFOBetweenCheckAndOpen(t *testing.T) {
	...
	case r := <-done:
		...
		if !errors.As(r.err, &nr) {
			t.Fatalf("Read = %+v, %v, want ErrNotRegular from the opened object", r.c, r.err)
		}
	case <-time.After(5 * time.Second):
		t.Fatal("Read blocked on the FIFO — the check and the open were two different objects")
	}
}

// failingDirEntry is a fs.DirEntry whose metadata read fails — permission
// denied or I/O on a real filesystem, forced here so the failure path is
// deterministic.
...
func (d failingDirEntry) Name() string               { return d.name }
func (d failingDirEntry) IsDir() bool                { return false }
func (d failingDirEntry) Type() fs.FileMode          { return 0 }
func (d failingDirEntry) Info() (fs.FileInfo, error) { return nil, d.err }

// TestEntryWithUnreadableMetadataIsDistinguishable: an entry whose metadata
// cannot be read must not become empty plausible data — it is KindUnreadable,
// distinguishable from a genuinely broken symlink, and refused by the
// openability table like any other unreadable object.
func TestEntryWithUnreadableMetadataIsDistinguishable(t *testing.T) {
	e := buildEntry(failingDirEntry{name: "x", err: fs.ErrPermission}, "/d", "/d")
	if e.Kind != filesystem.KindUnreadable {
		t.Fatalf("Kind = %q, want KindUnreadable", e.Kind)
	}
