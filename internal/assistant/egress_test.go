...
func TestMiddleware_EgressNoFindingReturnsByteForByte(t *testing.T) {
	...
	ref, refErr := executors["files.read"](context.Background(), cap, []byte(args))
	if refErr != nil {
...
func TestMiddleware_NewPolicyFailsClosedWithoutKnownMaterial(t *testing.T) {
	...
	if _, err := newPolicyMiddleware(grant, reg, &fakeLedger{}, NewApprovalStore(), nil, "run-1", 1, nil); err == nil {
