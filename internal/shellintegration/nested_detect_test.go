...
// TestZshNestedDetection is the zsh twin of TestBashNestedDetection: the
// nested-environment classifier must be conservative in zsh exactly as it
// is in bash — accepted forms only, everything ambiguous refused (which
// means: run conventionally). Same cases, same expectations: the two tiers
// share one conservative contract (nocx-u7uh.28).
func TestZshNestedDetection(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(fmt.Sprintf("%q", tc.line), func(t *testing.T) {
			// No capability: the script's init fails fast (cfg check) and
			// never blocks on a handshake; the detect latch is set after
			// sourcing, which is exactly the state the accept-line widget
			// sees.
			prog := fmt.Sprintf(`
export NOCX_SHELL_INTEGRATION=1
export NOCX_LIFECYCLE_LANE=lane-test
