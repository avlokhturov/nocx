...
// DirectoryPicker is the optional folder-picker half of the native dialog
// seam. Older/file-only adapters remain valid and report this method absent.
...
func (h dialogHandlers) handleDialogOpenDirectory(ctx context.Context, req jsonrpcRequest) {
	picker, ok := h.dialog.get().(DirectoryPicker)
	if !ok {
		_ = h.r.TryError(req.ID, RPCError{Code: -32601, Message: "native directory picker not available"})
		return
	}
	permit, ok := h.holdPicker(ctx, req)
	if !ok {
	...
	path, err := picker.OpenDirectory(ctx)
	if err != nil {
