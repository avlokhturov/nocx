...
- **Date:** 2026-08-02
- **Touches:** ADR-0035 (this decision), AD-7/AD-8 (session model and composition-root wiring),
...
> **Amended 2026-08-18:** ADR-0036, ADR-0038, and
...
> clauses. ADR-0039 and
> `docs/superpowers/specs/2026-08-18-sandbox-shell-contract-restoration-design.md` restore
> the original `Sandboxed shell…` product intent and supersede ADR-0037's fixed-OpenCode
...
- Error convention: `error.data.reason` strings ride `-32603`/`-32000` responses
  (`ws_vault.go`); `-32005`, `-32006`, `-32007` are unused in the tree (verified by search).
...
- The backend **rejects** a sandbox request while the flag is off (`-32005`), so UI and wire
...
1. The request must be local (`kind` empty, `profileId` empty, `host` empty) — a sandboxed SSH
   request is invalid.
2. The feature flag `sandbox.enabled` must be true — otherwise `-32005`
...
| -------- | ------------------------------------------------------------------- | ----------------------------- |
| `-32005` | feature disabled                                                    | `"feature-disabled"`          |
| `-32006` | backend unavailable (mirrors a `sandbox.status` reason)             | e.g. `"landlock-abi-too-old"` |
| `-32007` | setup failed (policy construction, helper handshake, native launch) | `"setup-failed"`              |
...
(fail closed, `-32007`). The real user home, broad `/tmp`, and user config/credential
...
- `cfg.Sandbox != nil` → `Service.Prepare(ctx, *cfg.Sandbox, spec)`; on success the
  ...
  error (`-32007`) and **no session is registered**.
...
- Require kernel Landlock ABI `>= 3`: below ABI 3 the kernel cannot deny truncation at all
  ...
  `landlock-unavailable` / `landlock-abi-too-old` (`-32006`).
...
- On timeout/error: close the PTY, terminate and wait for the helper, run cleanup once
  (runtime tree), return `-32007`. No session registry entry leaks (matches `Reg.Open`
  ordering, §1).
...
A profile/render/probe failure is fail-closed: `-32006`/`-32007`, no tab, no unsandboxed
fallback.
...
| flag off, `open` with `sandbox`                  | `-32005 feature-disabled`                                                                                |
...
| Landlock unavailable / ABI < 3                   | `-32006` with stable reason; Quick Connect shows `Sandbox unavailable` row                               |
| helper timeout / exit 126                        | `-32007`, no session, PTY closed, helper waited, cleanup once                                            |
| Seatbelt missing / profile rejected              | `-32006`/`-32007`, no tab, no fallback                                                                   |
