...
## 4. The engine: our loop, a replaceable model client

### 4.1 Why the loop is ours

ADR-0020 makes the loop part of the security boundary, and two of its requirements cannot be
delegated to a stock agent framework:

- **A refusal is a control decision, not a tool result.** Handing the model "permission
  denied" as an ordinary result lets it improvise around the refusal — retry, pick a broader
  tool, or encode the same operation as a shell command. That is a hint, not enforcement.
- **An escalation must stop the loop before any domain call**, obtain approval, and then
  **mint a new attempt with a new grant** — because ADR-0020 says approval may not mutate an
  executing run's grant. A framework does not model that distinction; it is ours.

A framework would also arrive owning things the ledger owns: when a run is finished, retries,
...
So: `internal/agent` owns the loop, the policy, the attempts and the tool dispatcher.

### 4.2 One loop, both rungs

```
assemble a projection from the ledger + referenced frames
  → stream a model response
  → accumulate text deltas and, separately, proposed tool calls
  → when a call completes: validate name and arguments
  → evaluate it against the run's immutable grant
        refuse  ·  suspend for approval  ·  permit
  → invoke the Go tool, record the attempt, append the result
...
Text deltas and proposed tool calls are **different event classes** and must never be merged
into "text". The arrival of a proposed call is the moment control passes to policy; merging
it into the text stream destroys the only point where the grant is checked.
...
the proposal **and** the run, with a recorded reason the user sees. Escalation, not silent
continuation, is what "ask instead of refuse" means.
...
the loop is in Go: the authority check is a call next to the thing it protects, not a message
across a pipe from it.
...
### 4.4 The model client

One interface — _give it messages and declared tools, get back a stream_. It knows nothing
about the ledger, grants or frames.

"Narrow" was wrong, though: the stream is not just text deltas and proposed calls. The loop
cannot decide **whether a response completed**, **whether a proposed call is whole**, or
**whether a retry is safe** without the rest of it — so the interface also carries the finish
reason (stopped, length, tool calls, filtered), tool-call ids, usage, the model actually
served, the provider's request id for diagnosis, a structured error, and whether that error is
retryable. These are not provider trivia; each one is a decision the loop has to make.

**The SSE client is ours, and it is the part most likely to be underestimated.** At minimum
it owns: HTTP status and error bodies _before_ a stream starts; SSE framing across arbitrary
TCP chunk boundaries, several events in one read and one event split across reads; UTF-8
split mid-codepoint; `[DONE]` versus an EOF with no completion; incremental assembly of tool
calls by index and id, with the name and the JSON arguments arriving in fragments; oversized
or malformed arguments; parallel calls; a trailing usage chunk; connect, header, idle and
total deadlines; cancellation that actually closes the body; bounded sizes for event, answer
and argument, with backpressure rather than unbounded buffering when the ledger or the
renderer is slower than the stream; retry allowed only before any content was observed;
redirects that never carry the credential to another origin; and errors that carry the
provider's request id without carrying the key.

**One implementation to start: OpenAI Chat Completions over HTTPS + SSE.** That single
protocol reaches:
...
No agent framework is taken as a dependency. A second implementation goes **behind this
interface**, and the expected one is **Anthropic Messages, at the agent rung** — not for the
provider but for the shape of the wire: it halts at `tool_use` and waits for `tool_result`,
so the protocol itself pauses exactly where §4.1's policy must take control. Reaching Claude
through an OpenAI-compatible aggregator loses precisely that. Any third-party adapter set is
judged by measurement — `go list -deps`, module size, stripped binary size — not by argument.
...
entries; the loop with **zero tools** and its streaming; the endpoint/model configuration with
the vault reference; masking extended into artifact ingest; the failure surfaces.
...
- **Tools.** The dispatcher and the grant check are designed here and exercised by the
  zero-tool case; the first real tool is the next slice, and it is the moment the lane and the
  lease become load-bearing.
...
- The engine is configured with **zero tools declared**, and a scripted model that proposes a
  tool call anyway has the call **refused by the dispatcher** and the run ended — not passed
  to any Go function. **And the paired positive:** with a reachable endpoint, an ask succeeds
  end to end.
...
5. **We own the SSE client** (§4.4). The list there is long and every item is a real branch;
   underestimating it is the most likely way this lands late.
6. **"OpenAI-compatible" is not one protocol.** Endpoints differ on streamed tool arguments,
   parallel calls and usage accounting. The first implementation targets chat + streaming
   text; tool calling is verified per endpoint before the agent rung claims support.
...
`pi --mode rpc` → pi's Node SDK in a sidecar → `adk-go` → `cloudwego/eino` → **our own loop
over a replaceable model client**. Each step was forced by a fact rather than a preference:
pi's client-callable `bash` and its deliberate absence of MCP (`docs/usage.md:303`) meant our
tools would have to live in TypeScript inside someone else's process; adk-go has no Anthropic
and no Ollama; eino has both but its stock ReAct loop cannot make refusal a control decision
or suspend for approval, which ADR-0020 requires. What decided it was the owner's requirement
that the assistant use **our** tools: if the tools are our domain objects, the loop belongs in
...
- **The engine (§4): high on shape, unmeasured on cost.** No dependency is taken, so the cost
  is our own code: an SSE client and a small state machine.
