...
> **Amended 2026-08-16:** ADR-0036/0037 and their accepted design specs supersede this
...
to reserved JSON-RPC codes `-32005`/`-32006`/`-32007`. `pty.NewLocal` builds the ordinary
...
`docs/decisions/0035-native-per-tab-filesystem-sandbox.md`.
...
- **Wire tokens are frozen:** setting key `sandbox.enabled`; request field
  ...
  method `sandbox.status`; dialog method `dialog.openDirectory`; error codes `-32005`
  (`feature-disabled`), `-32006` (backend status reason), `-32007` (`setup-failed`).
...
| `internal/sandbox/errors.go`                     | typed setup/status errors mapping to `-32005/11/12` reasons                            |
...
- `open` without `sandbox` → identical result `{sessionId, cwd}`; `open` with `sandbox` on an
  SSH request → `-32602`.
- Flag off + `sandbox` present → `-32005` with `data.reason: "feature-disabled"`.
- Invalid/relative/nonexistent/symlinked workspace → `-32602`; a valid symlink resolves to its
  ...
  value.
- Backend unavailable (stub service) → `-32006` with the stable reason; `sandbox.status`
...
- Setup failure (service returns typed error) → `-32007` `setup-failed`; the session registry
...
- `-32007`/`-32006`/`-32005` from `open` → toast with the reason, no tab, no lingering
...
- Behavior matrix (each row a test name):
  ...
  | flag off, `open` + `sandbox`                     | `-32005 feature-disabled`                                       |
  ...
  | Landlock unavailable / ABI < 3                   | `-32006` stable reason; unavailable row                         |
...
- Landlock ABI floor (3) and cap (8) appear verbatim in: spec §8.1, ADR-0035, this plan's
