...
- **Status:** evidence ledger for `.internal/specs/2026-08-02-native-filesystem-sandbox-design.md`,
  ADR-0035, and `.internal/plans/2026-08-02-native-filesystem-sandbox-v1.md`
...
- JSON-RPC error conventions: `newJSONRPCError` / `rpcErrorFor` / `newVaultError` with
  ...
  (`ws_vault.go:71-76`). Codes `-32005`, `-32006`, `-32007` are unused in the tree
...
| 11  | Proposed Docker/OCI path                                 | **Reject**                        | `docs/plans/docker-sandbox/*` is proposal-only in termic and container runtime is not selected for nocx (ADR-0035 "Alternatives")                                                                                                                                        |
